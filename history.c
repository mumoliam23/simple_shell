...
if (!home_dir)
return (NULL);
file_path = malloc(sizeof(char) * (_strlen(home_dir)
...
/**
* write_to_history - writes command history to a file
...
/**
* read_from_history - reads command history from a file
* @info: the parameter struct
*
...
* Return: Always 0
*/
int build_history_list(info_t *info, char *command, int linecount)
{
list_t *node = NULL;
if (info->history)
node = info->history;
add_node_end(&node, command, linecount);
if (!info->history)
info->history = node;
return (0);
}
